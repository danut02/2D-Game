...
LRESULT CGameApp::DisplayWndProc( HWND hWnd, UINT Message, WPARAM wParam, LPARAM lParam )
{
	...
	case WM_KEYDOWN:
		...
		case 0x52:
			//R
			m_pPlayer_rotate->Position2().x = m_pPlayer->Position().x;
			m_pPlayer_rotate->Position2().y = m_pPlayer->Position().y;

			m_pPlayer->Position().y = -900;
			m_pPlayer->Position().x = -900;
			break;
...
bool CGameApp::BuildObjects()
{
	...
	m_pPlayer_rotate = new CPlayer(m_pBBuffer);
	m_crate = new CPlayer(m_pBBuffer);
...
void CGameApp::SetupGameState()
{
	...
	m_pPlayer2->Position() = Vec2(400, 450);
	m_pPlayer_rotate->Position2() = Vec2(-300, -300);
...
void CGameApp::ReleaseObjects( )
{
	...
	if (m_pPlayer_rotate != NULL)
	{
		delete m_pPlayer_rotate;
		m_pPlayer_rotate = NULL;
	}
...
void CGameApp::FrameAdvance()
{
	...
	if ( m_LastFrameRate != m_Timer.GetFrameRate() )
	{
		...
		sprintf_s( TitleBuffer, _T(" Lives : %d || Score : %d "), m_pPlayer->lives, m_pPlayer->score  );
...
void CGameApp::collision()
{
	...
	if (m_pPlayer->Position().x + 100 >= m_pPlayer2->Position().x && m_pPlayer->Position().x - 100 <= m_pPlayer2->Position().x
		&& m_pPlayer->Position().y + 140 >= m_pPlayer2->Position().y) {
		m_pPlayer->bgY-=170;

	
		
		m_pPlayer->Velocity().y = -40;
...
void CGameApp::AnimateObjects()
{
	...
	m_crate->Update(m_Timer.GetTimeElapsed());
	
	m_pPlayer_rotate->Update(m_Timer.GetTimeElapsed());
}
...
void CGameApp::DrawObjects()
{
	...
	for (int i = 1;i <= 20;i++)
		m_imgBackground.Paint(m_pBBuffer->getDC(), 0, m_pPlayer->bgY + 600 * i);
	for(int i=1;i<=20;i++)
	m_imgBackground.Paint(m_pBBuffer->getDC(), 0, m_pPlayer->bgY - 600*i);
		...
		if (m_pPlayer->bgY != 0) {
			...
			m_imgBackground.Paint(m_pBBuffer->getDC(), 0, m_pPlayer->bgY );
